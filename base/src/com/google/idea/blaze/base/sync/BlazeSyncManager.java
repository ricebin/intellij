...
public class BlazeSyncManager {
                  ...
                  ProgressiveTaskWithProgressIndicator.builder(project, "Initiating project sync")
                      .submitTask(
                          indicator ->
                              Scope.root(
                                  context -> {
                                    setupScopes(syncParams, context, indicator);
  ...
  private static Task getRootInvocationTask(BlazeSyncParams params) {
    String taskTitle;
    if (params.syncMode() == SyncMode.STARTUP) {
      taskTitle = "Project opened";
    } else if (params.syncOrigin().equals(BlazeSyncStartupActivity.IDENTIFIER)) {
      taskTitle = "New project imported";
    } else {
      taskTitle = "User requested sync";
    }
    return new Task(taskTitle, Task.Type.BLAZE_SYNC);
  }

  private void setupScopes(
      BlazeSyncParams params, BlazeContext context, ProgressIndicator indicator) {
    BlazeUserSettings userSettings = BlazeUserSettings.getInstance();

    context
        .push(new ProgressIndicatorScope(indicator))
        .push(
            new ToolWindowScope.Builder(project, getRootInvocationTask(params))
