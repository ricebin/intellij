...
public class KotlinDebuggerRunner extends BlazeJavaDebuggerRunner {
  ...
  public RunContentDescriptor createContentDescriptor(
    ...
    // If the kotlinx-coroutines library is a transitive dependency of the target to debug, save the
    // path of the library to be used as a javaagent during bazel run and create a
    ...
    Optional<ArtifactLocation> libArtifact =
        KotlinxCoroutinesLibFinderHelper.findKotlinxCoroutinesLib(config);

    if (libArtifact.isPresent()) {
      // Get the path of the kotlinx coroutines library jar file.
      Optional<String> libAbsolutePath = getArtifactAbsolutePath(config, libArtifact.get());
      if (libAbsolutePath.isPresent()) {
        ((BlazeJavaRunProfileState) state).addKotlinxCoroutinesJavaAgent(libAbsolutePath.get());
